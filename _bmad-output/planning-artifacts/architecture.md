...
## Procurement Readiness Delta (2026-08-23)

Additive architecture deltas approved via Sprint Change Proposal `sprint-change-proposal-2026-08-23.md` (Epic 8). Existing decisions remain authoritative.

### Object Storage: Garage

- Garage (S3-compatible object storage, Rust, production-ready) is added to `infra/docker-compose.yml` with stable service name `garage`, following the AR-006 stable-name pattern.
- Selection rationale: MinIO Community Edition entered maintenance mode December 2025 and its repository was archived April 2026 (no Docker images since October 2025); SeaweedFS is overweight for the small-object image workload; RustFS remains alpha. AGPLv3 usage of unmodified Garage as a separate internal service is acceptable.
- Backend integrates through the S3-compatible API with endpoint, credentials, and bucket bound via typed properties; no hardcoded URLs. Image serving uses short-TTL presigned GET URLs generated by the backend. PostgreSQL stores only object keys/references. An optional health indicator follows the Epic 6 pattern.

### Data Model Additions

- `spareparts`: nullable `material_code` with global unique constraint (`uq_spareparts_material_code`), nullable `lead_time_hours NUMERIC(12,2)` supporting fractional durations, nullable `image_object_key`.
- New append-only table `sparepart_price_entries`: sparepart FK, `amount NUMERIC(19,4)` mapped to `BigDecimal`, `currency CHAR(3)` ISO-4217 default `'IDR'`, conditional `kurs_to_idr NUMERIC(18,6)` required when currency differs from IDR, normalized IDR amount, entering user, entry timestamp.
- New shift configuration tables: machine-group-level config with up to three shift window rows (start/end local wall-clock times, cross-midnight permitted) and a parallel machine-level override table. Resolution precedence: machine override wins; otherwise group config applies; read models expose the resolved source.
- Counter rate statistics are computed query-time from InfluxDB telemetry history and cached in Redis with explicit TTL (NFR-001a); no relational snapshot table in this delta.

### Backend Modules and Calculations

- `CounterRateEstimator`: rolling 30-day moving average of counting delta per effective operating hour from accepted InfluxDB telemetry; full-history fallback below 30 days of data; explicit insufficient-data state; window/staleness as typed configuration; rounding rules versioned with formula.
- `OperatingCalendarCalculator`: effective operating time from resolved shift config; cross-midnight windows supported; plant-local timezone from typed config (default `Asia/Jakarta`) used only at shift definition and display boundaries; computation in UTC instants per rule #252.
- Alert evaluation extends the Epic 4 path with a second condition: when projected depletion falls within the lead-time window, a distinct `PROCUREMENT_RISK` alert is created with duplicate prevention keyed by installation plus alert type; existing percentage-threshold alerts are untouched.
- Job-scope enforcement utility added to the auth/security module: procurement readiness mutations require job scope LEADER or above server-side, in addition to application role baseline — first concrete step of the FR-025 ABAC direction.

### API Contracts (`/api/v1`)

- `PATCH /spareparts/{id}` for material code and lead time
- `POST /spareparts/{id}/price-entries`, `GET /spareparts/{id}/price-entries`
- `POST /spareparts/{id}/image`, `DELETE /spareparts/{id}/image`
- `PUT /machine-groups/{id}/shift-config`, `PUT /machines/{id}/shift-config`, `DELETE /machines/{id}/shift-config`
- Projections embedded in machine-hub/installation read responses

### Testing Strategy Additions

- Testcontainers PostgreSQL proves migrations, global material-code uniqueness, and append-only price history behavior; no persistence mocks.
- Controlled-clock unit tests cover moving-average estimation, operating-calendar math including cross-midnight shifts, and projection boundaries.
- Integration tests prove procurement-risk alert creation, dedupe, non-interference with threshold alerts, missing-data no-op, and LEADER-scope denial paths.
- DTO contracts locked via API contract tests; Orval/TanStack clients regenerated after OpenAPI updates.
