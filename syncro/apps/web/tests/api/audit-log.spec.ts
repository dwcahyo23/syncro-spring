...
const AUDIT_LOG_URL = `${API_BASE}/api/v1/audit-log`;
...
test.describe("Story 2.9 API client contract: audit log", () => {
  ...
  test("[P0] unauthenticated request returns 401", async ({ request }) => {
    const response = await request.get(AUDIT_LOG_URL);
  ...
  test("[P0] SUPER_ADMIN lists entries with the full response shape", async ({ request }) => {
    const response = await request.get(AUDIT_LOG_URL, { headers: authHeaders("SUPER_ADMIN") });
  ...
  test("[P1] filter parameters bind without error", async ({ request }) => {
    const response = await request.get(
      `${AUDIT_LOG_URL}?entityType=MACHINE&actor=admin&page=0&size=25&sort=createdAt,desc`,
  ...
  test("[P1] MANAGE role can read global master data audit entries", async ({ request }) => {
    const response = await request.get(AUDIT_LOG_URL, { headers: authHeaders("MANAGE") });
    expect(response.status()).toBe(200);
  });

  test("[P1] VIEWER role can read the audit log", async ({ request }) => {
    const response = await request.get(AUDIT_LOG_URL, { headers: authHeaders("VIEWER") });
    expect(response.status()).toBe(200);
  });

  test("[P1] unknown entityType returns 400 INVALID_QUERY_VALUE", async ({ request }) => {
    const response = await request.get(`${AUDIT_LOG_URL}?entityType=FOO`, { headers: authHeaders("SUPER_ADMIN") });
  ...
  test("[P1] invalid sort property returns 400 INVALID_QUERY_VALUE", async ({ request }) => {
    const response = await request.get(`${AUDIT_LOG_URL}?sort=plantId,asc`, { headers: authHeaders("SUPER_ADMIN") });
    expect(response.status()).toBe(400);
    await expectErrorCode(response, "INVALID_QUERY_VALUE");
  });

  test("[P1] malformed from date returns 400 INVALID_QUERY_VALUE", async ({ request }) => {
    const response = await request.get(`${AUDIT_LOG_URL}?from=not-a-date`, { headers: authHeaders("SUPER_ADMIN") });
    expect(response.status()).toBe(400);
    await expectErrorCode(response, "INVALID_QUERY_VALUE");
  });

  test("[P1] malformed to date returns 400 INVALID_QUERY_VALUE", async ({ request }) => {
    const response = await request.get(`${AUDIT_LOG_URL}?to=not-a-date`, { headers: authHeaders("SUPER_ADMIN") });
    expect(response.status()).toBe(400);
    await expectErrorCode(response, "INVALID_QUERY_VALUE");
  });

  test("[P1] no write endpoints exist for the audit log", async ({ request }) => {
    await expectStatus(request.put(AUDIT_LOG_URL, { headers: authHeaders("SUPER_ADMIN"), data: {} }), 405);
    await expectStatus(request.delete(AUDIT_LOG_URL, { headers: authHeaders("SUPER_ADMIN") }), 405);
  });
});
