...
test.describe("Story 2.5 ATDD API RED-PHASE scaffold: manage spareparts", () => {
  test.skip("2.5-ATDD-API-001 P0 SUPER_ADMIN creates sparepart with CATEGORY, BRAND, KIND, TYPE taxonomy refs and sees it in list", async ({
    ...
    const createResponse = await request.post(SPAREPARTS_URL, { headers: authHeaders("SUPER_ADMIN"), data: payload });
    ...
    const listResponse = await request.get(SPAREPARTS_URL, { headers: authHeaders("SUPER_ADMIN") });
  ...
  test.skip("2.5-ATDD-API-002 P0 MANAGE creates sparepart and VIEWER can read list/detail as global master data", async ({
    ...
    const createResponse = await request.post(SPAREPARTS_URL, { headers: authHeaders("MANAGE"), data: payload });
    expect(createResponse.status()).toBe(201);
    const created = await createResponse.json();

    const listResponse = await request.get(SPAREPARTS_URL, { headers: authHeaders("VIEWER") });
    expect(listResponse.status()).toBe(200);
    await expectListContains(listResponse, payload.code);

    const detailResponse = await request.get(`${SPAREPARTS_URL}/${created.id}`, { headers: authHeaders("VIEWER") });
  ...
  test.skip("2.5-ATDD-API-003 P0 rejects create when required taxonomy references are missing, null, unknown, or wrong dimension", async ({
    ...
    await expectStatus(
      request.post(SPAREPARTS_URL, { headers: authHeaders("MANAGE"), data: { ...sparepartPayload(), brandId: null } }),
      400,
    );
    await expectStatus(
      request.post(SPAREPARTS_URL, {
  ...
  test.skip("2.5-ATDD-API-004 P0 rejects update when taxonomy refs are unknown or wrong dimension using safe not-found or validation shape", async ({
    ...
    await expectStatus(
      request.put(`${SPAREPARTS_URL}/${sparepartId}`, {
  ...
  test.skip("2.5-ATDD-API-005 P0 rejects blank code/name, over-length values, malformed JSON, and invalid UUID values safely", async ({
    ...
    await expectStatus(request.get(`${SPAREPARTS_URL}/not-a-uuid`, { headers: authHeaders("MANAGE") }), 400);
  });
  ...
  test.skip("2.5-ATDD-API-006 P0 rejects duplicate code or name regardless casing and surrounding whitespace on create and update", async ({
    ...
    await expectConflict(
      request.put(`${SPAREPARTS_URL}/00000000-0000-4000-8000-000000000027`, {
        headers: authHeaders("SUPER_ADMIN"),
  ...
  test.skip("2.5-ATDD-API-008 P0 lists spareparts with predictable ordering and filters by category, brand, kind, type, and code/name search", async ({
    ...
    await expectStatus(
      request.get(
        `${SPAREPARTS_URL}?categoryId=11111111-1111-4111-8111-111111111111&brandId=22222222-2222-4222-8222-222222222222`,
    ...
    await expectStatus(
      request.get(
        `${SPAREPARTS_URL}?kindId=33333333-3333-4333-8333-333333333333&typeId=44444444-4444-4444-8444-444444444444`,
        { headers: authHeaders("VIEWER") },
      ),
      200,
    );
    await expectStatus(request.get(`${SPAREPARTS_URL}?search=bearing`, { headers: authHeaders("VIEWER") }), 200);
    await expectStatus(request.get(`${SPAREPARTS_URL}?search=BRG-6205`, { headers: authHeaders("VIEWER") }), 200);
  });
  ...
  test.skip("2.5-ATDD-API-009 P0 updates bounded fields and taxonomy references then returns updated taxonomy labels", async ({
    ...
    const updateResponse = await request.put(`${SPAREPARTS_URL}/${sparepartId}`, {
      headers: authHeaders("MANAGE"),
      data: payload,
    });
    ...
    const detailResponse = await request.get(`${SPAREPARTS_URL}/${sparepartId}`, { headers: authHeaders("MANAGE") });
    expect(detailResponse.status()).toBe(200);
  ...
  test.skip("2.5-ATDD-API-010 P0 deletes sparepart without dependent installations and subsequent get returns safe not found", async ({
    ...
    await expectStatus(
      request.delete(`${SPAREPARTS_URL}/${sparepartId}`, { headers: authHeaders("SUPER_ADMIN") }),
      204,
    );
    await expectStatus(request.get(`${SPAREPARTS_URL}/${sparepartId}`, { headers: authHeaders("SUPER_ADMIN") }), 404);
  });
  ...
  test.skip("2.5-ATDD-API-011 P0 returns safe 409 conflict when deleting sparepart referenced by installed spareparts", async ({
    ...
    await expectConflict(
      request.delete(`${SPAREPARTS_URL}/${installedSparepartDependencyTargetId}`, { headers: authHeaders("MANAGE") }),
    );
  });
  ...
  test.skip("2.5-ATDD-API-012 P0 denies VIEWER create, update, and delete with safe forbidden response", async ({
    ...
    await expectStatus(
      request.put(`${SPAREPARTS_URL}/00000000-0000-4000-8000-000000000031`, {
        headers: authHeaders("VIEWER"),
    ...
    await expectStatus(
      request.delete(`${SPAREPARTS_URL}/00000000-0000-4000-8000-000000000031`, { headers: authHeaders("VIEWER") }),
      403,
    );
  });
  ...
  test.skip("2.5-ATDD-API-013 P0 unauthenticated requests to every sparepart endpoint return existing safe authentication error shape", async ({
    ...
    await expectStatus(request.get(SPAREPARTS_URL), 401);
    await expectStatus(request.get(`${SPAREPARTS_URL}/00000000-0000-4000-8000-000000000032`), 401);
    await expectStatus(
      request.post(SPAREPARTS_URL, { data: sparepartPayload({ code: "NOAUTH-01", name: "No Auth Create" }) }),
      401,
    );
    await expectStatus(
      request.put(`${SPAREPARTS_URL}/00000000-0000-4000-8000-000000000032`, {
        data: sparepartPayload({ code: "NOAUTH-02", name: "No Auth Update" }),
      }),
      401,
    );
    await expectStatus(request.delete(`${SPAREPARTS_URL}/00000000-0000-4000-8000-000000000032`), 401);
  });
  ...
  test.skip("2.5-ATDD-API-014 P1 returns global non-plant-scoped sparepart master data for MANAGE and VIEWER despite plant assignment limits", async ({
    ...
    await expectStatus(request.get(SPAREPARTS_URL, { headers: authHeaders("MANAGE") }), 200);
    await expectStatus(request.get(SPAREPARTS_URL, { headers: authHeaders("VIEWER") }), 200);
  });

  test.skip("2.5-ATDD-API-015 P1 rejects invalid UUID query filters and unknown sparepart detail id safely", async ({
    request,
  }) => {
    await expectStatus(request.get(`${SPAREPARTS_URL}?categoryId=not-a-uuid`, { headers: authHeaders("VIEWER") }), 400);
    await expectStatus(
      request.get(`${SPAREPARTS_URL}/99999999-9999-4999-8999-999999999999`, { headers: authHeaders("VIEWER") }),
      404,
    );
  });
});
