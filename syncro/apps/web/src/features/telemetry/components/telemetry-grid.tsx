...
export function MachineTelemetryCard({ machine }: { machine: TelemetryMachineView }) {
  ...
  return (
    <Card aria-label={`Telemetry for machine ${machine.code}`} className="h-full">
      <CardHeader className="gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {freshness ? <StatusBadge freshness={freshness} /> : <Badge variant="outline">No data received</Badge>}
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        {telemetry ? (
          ...
          <p className="text-muted-foreground text-sm">No data received yet for this machine.</p>
        )}
        {optionalEntries.length > 0 ? (
          <dl aria-label="Optional telemetry fields" className="grid gap-1">
