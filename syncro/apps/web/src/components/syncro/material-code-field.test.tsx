...
describe("MaterialCodeField", () => {
  it("renders the value and emits changes", () => {
    ...
    renderI18n(<MaterialCodeField value="MC-001" onChange={onChange} />);
  ...
  it("shows the error message with alert role when provided", () => {
    renderI18n(<MaterialCodeField value="" onChange={() => undefined} error="Already used by another sparepart." />);
  ...
  it("disables the input in read-only mode and shows the disabled reason instead of the hint", () => {
    renderI18n(<MaterialCodeField value="MC-LOCKED" onChange={() => undefined} readOnly disabledReason="View only." />);
  ...
  it("caps input length at 64 characters", () => {
    renderI18n(<MaterialCodeField value={Array.from({ length: 64 }, () => "a").join("")} onChange={() => undefined} />);
