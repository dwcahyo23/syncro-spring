-- ============================================================================
-- V4: Work-log audit label (story 17-2, blueprint B4, AD-18)
-- Additive-only migration on top of the V1 baseline + V3 extension. Extends the
-- ck_audit_log_entity_type CHECK with the WORK_LOG value that
-- AuditEntityType.WORK_LOG writes (the doc comment on that enum requires the
-- Java enum and the SQL CHECK to move together in one change).
