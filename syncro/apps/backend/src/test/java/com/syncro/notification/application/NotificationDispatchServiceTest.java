...
class NotificationDispatchServiceTest {
  ...
  void setUp() {
    ...
    service = new NotificationDispatchService(jobRepository, attemptRepository, wahaClient,
        templateRenderer, clock);
  }

  private NotificationJobEntity buildJob(int attemptCount) {
    ...
    // reflectively set id and attemptCount via repeated markAttemptFailed simulation
    // use real constructor — id is generated by JPA so use spy approach via overriding
    return job;
  }
  ...
  void dispatch_whenWahaFailsAndAttemptsRemain_staysAsPendingWithNextAttemptAt() {
    var job = new NotificationJobEntity(
        ...
        UUID.randomUUID(), RECIPIENT_PHONE,
        ALERT_ID + "::TECHNICIAN", TRACE_ID, null);
    // maxAttempts defaults to 3, attemptCount starts at 0
  ...
  void dispatch_whenWahaFailsAtMaxAttempts_marksJobExhausted() {
    ...
    // Simulate 2 prior failures so attemptCount=2, one more will exhaust (maxAttempts=3)
