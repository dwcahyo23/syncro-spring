...
class NotificationWorkerStatusControllerTest {
  ...
  void superAdminGetsNotificationWorkerStatus() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/notification/worker/status")
        ...
        .andExpect(jsonPath("$.lastFailedAlertId").value(nullValue()));
  }

  @Test
  void superAdminSeesLastFailedAlertIdWhenPresent() throws Exception {
    UUID alertId = UUID.fromString("00000000-0000-0000-0000-000000000002");
    when(statusService.status()).thenReturn(new NotificationWorkerStatus(
        NotificationWorkerState.DEGRADED,
        "Degraded",
        "WARNING",
        "WAHA circuit breaker is OPEN",
        "2026-08-21T08:00:00Z",
        "2026-08-21T07:59:00Z",
        null,
        2,
    ...
    mockMvc.perform(get("/api/v1/notification/worker/status")
            .with(auth(user(ApplicationRole.SUPER_ADMIN))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.lastFailedAlertId").value(alertId.toString()))
        .andExpect(jsonPath("$.lastFailureReason").value("HTTP 500: internal error"))
