...
class V1BaseSchemaMigrationTest {
  ...
  void migrationsApplied() {
    ...
    assertThat(rows.get(7).get("version")).isEqualTo("8");
    assertThat(rows.get(7).get("script")).isEqualTo("V8__pm_checklist_audit_types.sql");
    assertThat(rows.get(7).get("success")).isEqualTo(true);
    assertThat(rows.get(8).get("version")).isEqualTo("9");
    assertThat(rows.get(8).get("script")).isEqualTo("V9__pm_schedule_audit_types.sql");
    assertThat(rows.get(8).get("success")).isEqualTo(true);
  ...
  /**
   * Story 19-2: V8 extends the CHECK additively with PM_CHECKLIST_CATEGORY/
   * PM_CHECKLIST_ITEM so {@code AuditEntityType.PM_CHECKLIST_CATEGORY} /
   * {@code AuditEntityType.PM_CHECKLIST_ITEM} can be persisted. Every prior value —
   * including the V7 PM_FREQUENCY/PM_CHECKSHEET pair — survives the extension.
   */
  @Test
  @DisplayName("19.2-DB-001 P0 audit_log entity_type CHECK accepts PM_CHECKLIST_CATEGORY/PM_CHECKLIST_ITEM after V8")
  void auditEntityTypeAcceptsPmChecklistTypes() {
    var check = jdbc.queryForMap(
        "SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint "
            + "WHERE conname = 'ck_audit_log_entity_type'");
    var def = (String) check.get("def");
    assertThat(def).contains("PM_CHECKLIST_CATEGORY", "PM_CHECKLIST_ITEM");
    // every prior value survives the additive V8 extension.
    assertThat(def).contains("PM_FREQUENCY", "PM_CHECKSHEET",
  ...
  /**
   * Story 19-3: V9 extends the CHECK additively with PM_SCHEDULE/PM_SCHEDULE_DATE so
   * {@code AuditEntityType.PM_SCHEDULE} / {@code AuditEntityType.PM_SCHEDULE_DATE} can
   * be persisted. Every prior value — including the V8 PM_CHECKLIST_CATEGORY/
   * PM_CHECKLIST_ITEM pair — survives the extension.
   */
  @Test
  @DisplayName("19.3-DB-002 P0 audit_log entity_type CHECK accepts PM_SCHEDULE/PM_SCHEDULE_DATE after V9")
  void auditEntityTypeAcceptsPmScheduleTypes() {
    var check = jdbc.queryForMap(
        "SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint "
            + "WHERE conname = 'ck_audit_log_entity_type'");
    var def = (String) check.get("def");
    assertThat(def).contains("PM_SCHEDULE", "PM_SCHEDULE_DATE");
    // every prior value survives the additive V9 extension.
    assertThat(def).contains("PM_CHECKLIST_CATEGORY", "PM_CHECKLIST_ITEM",
  ...
  /**
   * Story 19-4: V10 extends the CHECK additively with PM_WORK_ORDER so
   * {@code AuditEntityType.PM_WORK_ORDER} can be persisted. Every prior value —
   * including the V9 PM_SCHEDULE/PM_SCHEDULE_DATE pair — survives the extension.
   */
  @Test
  @DisplayName("19.4-DB-001 P0 audit_log entity_type CHECK accepts PM_WORK_ORDER after V10")
  void auditEntityTypeAcceptsPmWorkOrder() {
    var check = jdbc.queryForMap(
        "SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint "
            + "WHERE conname = 'ck_audit_log_entity_type'");
    var def = (String) check.get("def");
    assertThat(def).contains("PM_WORK_ORDER");
    // every prior value survives the additive V10 extension.
    assertThat(def).contains("PM_SCHEDULE", "PM_SCHEDULE_DATE",
  ...
  /**
   * Story 19-4: V10 creates the partial unique index uq_pm_work_orders_period on
   * pm_work_orders(machine_id, template_id, scheduled_date) WHERE both non-null —
   * the race-safe idempotency backstop for workorder generation (FR-134).
   */
  @Test
  @DisplayName("19.4-DB-002 P0 partial unique index uq_pm_work_orders_period exists after V10")
  void pmWorkOrderPeriodIndexExists() {
    var index = jdbc.queryForMap(
        "SELECT indexdef FROM pg_indexes WHERE indexname = 'uq_pm_work_orders_period'");
    var def = (String) index.get("indexdef");
  ...
  /**
   * Story 21-1: V13 extends the CHECK additively with NON_CONFORMANCE +
   * EIGHT_D_REPORT so the compliance audit types can be persisted. Every prior
   * value — including the V12 KPI_TARGET — survives the extension.
   */
  @Test
  @DisplayName("21.1-DB-001 P0 audit_log entity_type CHECK accepts NON_CONFORMANCE/EIGHT_D_REPORT after V13")
  void auditEntityTypeAcceptsComplianceTypes() {
    var check = jdbc.queryForMap(
        "SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint "
            + "WHERE conname = 'ck_audit_log_entity_type'");
    var def = (String) check.get("def");
    assertThat(def).contains("NON_CONFORMANCE", "EIGHT_D_REPORT");
    // every prior value survives the additive V13 extension.
    assertThat(def).contains("KPI_TARGET", "PM_EXECUTION", "PM_EXECUTION_ITEM", "PM_WORK_ORDER",
        "WORK_ORDER_QUALITY_RATING", "WORK_LOG_RATING", "WORK_LOG", "WORK_ASSIGNMENT",
        "PLANT", "MACHINE", "ALERT", "SPAREPART_REQUEST", "SIGNATURE_USE");
  }

  /**
   * Story 21-1: V13 adds the nullable severity column to non_conformances with an
   * uppercase CHECK (MINOR/MAJOR/CRITICAL) — the story AC's severity requirement.
   */
  @Test
  @DisplayName("21.1-DB-002 P0 non_conformances carries severity + ck_non_conformances_severity after V13")
  void ncSeverityColumnAndCheck() {
    assertThat(columnNames("non_conformances")).contains("severity");
    var check = jdbc.queryForMap(
        "SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint "
            + "WHERE conname = 'ck_non_conformances_severity'");
