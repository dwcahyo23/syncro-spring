...
**WAHA reality check (placeholder numbers).** The seed's recipient numbers (`6281234567801/02/03`) are placeholders. Because the seed assigns each responsibility a WhatsApp number, routing always succeeds — `ROUTING_FAILED` only occurs when routing itself fails (no assignment/user/whatsapp). The realistic live-WAHA outcome for a threshold alert with placeholder numbers is `PENDING` → `SENT` with attempt evidence, then repeated `HTTP 500 no LID found` responses from the GOWS engine (deferred-work DW-88) until the job reaches `EXHAUSTED` after `maxAttempts=3`. During the pilot, one placeholder send was observed reaching `SENT` (WAHA returned HTTP 201 and the backend logged `send attempt ... status=201` + `sent successfully`) — WAHA accepted the message and handed it to WhatsApp, which is the strongest delivery evidence this pilot captures. This is **not** a verified production delivery guarantee: the recipient number is unregistered, the follow-up retry returned `500 no LID found`, and no on-device read receipt was observed. Replace the placeholder numbers with real numbers in `pilot-seed.sql` (and re-seed) before a live WAHA delivery pilot.
...
| **SM-005** | A WhatsApp notification is sent through WAHA for the threshold alert. | **Demonstrated up to the queue + attempt evidence, with one observed 201-accepted send.** A TECHNICIAN `notification_jobs` row is created and the worker attempts the WAHA send; one placeholder send reached `SENT` (WAHA HTTP 201, `send attempt ... status=201`, `sent successfully`). With the seed's placeholder numbers, the expected live-WAHA outcome is `PENDING`/`SENT` with attempt evidence, then `EXHAUSTED` after retries — a verified real-device delivery is not asserted (see section 6). |
