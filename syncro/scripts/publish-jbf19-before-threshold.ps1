<#
.SYNOPSIS
    Publishes the pilot before-threshold MQTT fixture (counting=890, 89.00% < 90%) to the local
    EMQX broker through the EMQX Management API.
...
.DESCRIPTION
    ...
    "pilot-jbf19-before-threshold-890" - stays verbatim: the payload is regex-substituted on the
    raw file text and is NEVER re-serialized through ConvertFrom-Json/ConvertTo-Json.
    ...
    Expected backend outcome (story 7-4 proves it end-to-end): counting=890 -> 89.00% < 90%
    threshold -> telemetry accepted, NO sparepart alert.

.EXAMPLE
    powershell -NoProfile -File syncro/scripts/publish-jbf19-before-threshold.ps1

    Publishes the before-threshold fixture with a refreshed UTC timestamp to
    factory/GM1/BF-08410/telemetry using syncro/.env connection values.

.EXAMPLE
    powershell -NoProfile -File syncro/scripts/publish-jbf19-before-threshold.ps1 -NoTimestampRefresh
...
.NOTES
    ...
    running for the publish itself; a subscribed backend ingests it as counting=890 telemetry
    (verify with syncro/scripts/verify-pilot.ps1).
    ...
    duplicate-publish AC. After the window, the alert-level dedup guard and the notification-job
    idempotency key keep a republish harmless.
...
if (-not $FixtureFile) { $FixtureFile = Join-Path $SyncroRoot 'tests\fixtures\mqtt-jbf19-before-threshold-payload.json' }
...
Write-Host '=== Publish pilot fixture (before threshold) ==='
...
if ($brokerMessage -eq 'no_matching_subscribers') {
    ...
    Write-Host '      backend never receives it: no delayed state surprise.'
}
